...
const formatNumber = (num) => {
  ...
  return padded.split('').map(digit => egyptianNumbers[digit]).join('');
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        <div className="flex relative flex-col lg:flex-row gap-8">
          <div className="flex-1">
           ...
           <h2 className="text-[20px] lg:text-[35px] font-bold text-[#8a3c2c] mb-4 lg:max-w-[20%] font-ghaith">{service.title}</h2>
            <p className="text-lg text-gray-800 leading-relaxed w-[90%] lg:max-w-[65%] whitespace-pre-line">
              {service.description}
            </p>
           </div>
          </div>
           {/* Arrow link */}
           <a href="#" className="absolute left-0 bottom-0 flex items-center justify-center w-8 lg:w-12 h-8 lg:h-12 bg-white rounded-full text-white transition-all duration-300 group-hover:scale-110 z-20">
...
const Services2 = () => {
  ...
  return (
    <div className={`${styles.padding} bg-[#f9f7ee]`}>
